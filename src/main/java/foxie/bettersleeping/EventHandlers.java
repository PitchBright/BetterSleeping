...
public class EventHandlers
{
	...
	public void onPlayerTick(TickEvent.PlayerTickEvent event)
	{
		...
		// long bedTime = data.getDayTicksAtLastLogOff() + (data.getSleepLevel()
		// - 6000);
		// long bedTime = (((event.player.worldObj.getWorldTime() % 24000) +
		// ((data.getSleepLevel() - 6000) % 24000)) % 24000);
		// long wakeTime = ((event.player.worldObj.getWorldTime() % 24000) +
		// (((24000 - data.getSleepLevel()) / 3) % 24000));
		// long logTime = (data.getDayTicksAtLastLogOff() % 24000);
		// long curTime = (event.player.worldObj.getWorldTime() % 24000);
		// BSLog.info("Curr: %d, Enr: %d, Log: %d, Bed: %d, Wake: %d", curTime,
		// data.getSleepLevel(), logTime, bedTime, wakeTime);
	...
	public void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event)
	{
		...
		if(energy - missedTime24 > 6000)
		{
			...
			// System.out.println("Scenario 1.1 Player joins during standard
			// energy reduction");
		}
		else
		{
			if(energy - missedTime24 > 0)
			{
				...
				heartsToGive++;
				// System.out.println("Scenario 2.1 Player joins during
				// BedTime:" + (6000 - (energy - missedTime24)));
			}
			else
			{
	...
	public void onPlayerLoggedOut(PlayerEvent.PlayerLoggedOutEvent event)
	{
		...
		data.setLoggedOff(event.player.worldObj.getTotalWorldTime(), event.player.worldObj.getWorldTime()); // CAFFEINE
																											// NEEDS
																											// TOTAL-WORLD-TIME
		// data.setLoggedOff(event.player.worldObj.getWorldTime(),
		// event.player.worldObj.getWorldTime()); //doesn't work with
		// Total-World-Time
